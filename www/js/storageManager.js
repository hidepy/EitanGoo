...
convStorage2Hash -private
	STORAGE_KEY_NAMEをキーとし、localstorageから保存されたデータを取得する
...
!function(proto){
	...
	proto.deleteItems = function(keys){
		...
		window.localStorage.setItem(STORAGE_KEY_NAME, JSON.stringify(_items));
	};
	...
	proto.deleteAllItem = function(){
		window.localStorage.setItem(STORAGE_KEY_NAME, JSON.stringify({}));
	};
